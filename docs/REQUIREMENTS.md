...
- **利用シナリオ:** 1. 代表者（ホスト）がスマホでブラウザを開き、「ルーム」を作成する。
  2. ルームID（または共有リンク／QRコード）を他の参加者（ゲスト）に共有する。※ コード・リンクのコピーは `SharePanel` に実装済み。QR モーダルは未実装（`TODO.md` MVP 残タスク）。
...
- [x] **ホスト設定機能**
  - ホストはロビーで次の項目を設定でき、変更は全員のロビー画面にリアルタイム反映される。
    - ルール種別（通常のみオンライン対戦で有効。特殊ルールはアルゴリズム実装済み・統合は未着手）／NORMAL のマッチ終了方式（脱落式・1ラウンド確定）／ラウンド制限時間（秒）／次ラウンド進行モード（自動・手動の切り替え）／1マッチ内のあいこ回数上限
  - 進行モードが「手動」の場合、ホストの操作で次ラウンド・次フェーズへ進める。
- [x] **途中参加・再接続の扱い**
  - ゲーム進行中の新規参加者は観戦として入室し、次のマッチからプレイ参加する。
  - 通信が切断されたプレイヤーは、そのマッチ終了まで生存状態を維持し、同一トークンでの再接続により状態を復元して復帰できる。マッチ進行中の意図的な退室（`LEAVE`）も切断と同等に扱い、そのマッチ終了まで生存を維持する（ロビー／マッチ終了時の退室は即時）。※ `LEAVE` は WS で実装済み。フロントの退室・ルーム移動 UI は未実装（`TODO.md` MVP 残タスク）。
...
> **現状**: `game/rules/minority.py`・`boss_battle.py`・`tournament.py` と `game/draw_resolution.py` に**判定アルゴリズムとあいこ上限処理**を実装済み（単体テスト通過）。`RoundRunner` は **NORMAL のみ**稼働。フロントは特殊ルールを選択不可（`SettingsPanel`）。プレイ可能にする統合作業は `TODO.md`「特殊ルール：ランタイム統合」（Step **R0–R6**）・`ARCHITECTURE.md` §5.1 を参照。

- [ ] **少数派勝利ルール (Minority Rule)** ※アルゴリズムのみ実装済み
