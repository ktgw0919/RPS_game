...
参加者 (4/20)              [ ＋CPU（ランダム） ]
  ...
  • 🤖 CPU-1 [ 削除 ]
      手の列: ✊ ✌️（ラウンドごとに繰り返し）
      [ 単発 ✊✌️✋ ]  [ 列に追加 ✊✌️✋ ] [ 列を適用 ] [ 下書きクリア ]
  [ ＋CPU ✊✌️✋ ]  ← 1手だけ指定して FIXED CPU を追加（ホストのみ）
────────────────────────────────
ゲーム設定 〔ホストのみ編集可〕
  ルール       [ 通常 ▼ ]
...
- 特殊ルール（少数派・代表・トーナメント）はロビー設定・ゲーム画面から選択・プレイ可能（`TODO.md` R6 完了）。
...
- **エラー導線**: ルーム無し / 満員 / 解散 / トークン不正は `ERROR`（コード付き）を受けてホームへ誘導し、理由を表示する（解散時は「ルームは解散しました（長時間無操作など）」等の日本語メッセージ）。ルーム内の非致命エラー（開始条件未達・状態不正など）は `SessionNotices` に表示し、ホスト操作は WebSocket 未接続時に無効化する。
- **開発/デモ用 CPU プレイヤー**: ロビーの参加者エリアに、ホストのみ・`ALLOW_CPU` 有効時のみ次を表示する。
  - **追加**: 「＋CPU（ランダム）」（`ADD_CPU`・`strategy=RANDOM`）、または ✊✌️✋ ボタン行（`ADD_CPU`・`strategy=FIXED`・単発1手で追加）
  - **手列編集**（各 CPU 行・ロビーのみ）: 現在の手列表示、「単発」で即 `UPDATE_CPU`、「列に追加」＋「列を適用」で下書きから手列を送信、`REMOVE_CPU` で削除
  - CPU は参加者一覧で**ロボットバッジ（🤖）**付き。定員・開始最小人数にカウントされる。`ALLOW_CPU=false` では追加 UI を出さず、サーバーは `CPU_NOT_ALLOWED`（`ARCHITECTURE.md` §3/§4.1/§11）
