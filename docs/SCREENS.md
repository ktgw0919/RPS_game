...
| └ ロビー               | `Room.status = WAITING`      | 参加者一覧・コード / QR 共有・ホスト設定パネル・開始ボタン（ホスト） |
...
ロビーを縦積みで構成し、共有エリア → 参加者一覧 → 設定パネル → 開始ボタンの順に並べる（区切り線で各ブロックを分割）。

```text
共有エリア
  ルーム: AB3K        [QR] [コピー]   ← QR は未実装（コード・リンクコピーは実装済み）
────────────────────────────────
...
| --------------------------- | ---------------------- | ----------------------------------- | ------ | ------------------------------------------------------------------------------ |
| `rule_type`                 | セレクト or セグメント | 通常 / 少数派 / 代表 / トーナメント | 通常   | 常時。MVP は通常のみ有効、**他は非活性表示**（「Phase 3」ラベル付きで選択不可） |
...
- 将来ルール（少数派 / 代表 / トーナメント）は MVP では**選択肢として表示するが非活性**（実装は Phase 3 以降）。
...
- 機能要件（ルーム管理・ホスト設定・途中参加 / 再接続）: `REQUIREMENTS.md` §3
