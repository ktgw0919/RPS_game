...
│       │   ├── cpu.py            # CPU プレイヤーの手生成 (CpuStrategy: RANDOM / FIXED)
...
- **CPU プレイヤー（開発/デモ用）**: CPU は**トークン・WebSocket 接続を持たないサーバー制御プレイヤー**で、`JOIN` 検証・再接続・`SESSION_REPLACED`・ハートビートの対象外。`ConnectionManager` に接続エントリを持たないため、ブロードキャストの送信先にならず `STATE_SYNC` も送られない。CPU は**ホストにならず**、ホスト自動移譲（§10）の候補からも除外する。追加/削除・手列設定はホストの WS 操作（§4 `ADD_CPU` / `UPDATE_CPU` / `REMOVE_CPU`）で行い、`ALLOW_CPU=false` のときは拒否する（§4.1 `CPU_NOT_ALLOWED`）。
...
- **`GET /rooms/{code}/matches`（Phase 5）**
  ...
  - **`MatchHistoryEntry`**（各要素。時刻は §4 規約の ISO8601 文字列）:
    - `{ match_id, rule_type, players: [{ player_id, display_name, is_cpu }], winner_ids, scores, started_at, ended_at }`（将来 `rounds: RoundLogEntry[]` を追加予定・§5.2）
...
全メッセージは封筒形式 `{ "type": T, "payload": P, "v": 1 }`。以下の `payload` 列は TypeScript 風の表記で示し、`frontend/src/types` と Pydantic v2 モデルの双方で同一構造を定義する。`Hand = "ROCK" | "SCISSORS" | "PAPER"`。`CpuStrategy = "RANDOM" | "FIXED"`（`RANDOM` は毎ラウンド乱数、`FIXED` は開発/デバッグ用の手列再生。後フェーズで mimic / 難易度などを追加予定）。
...
| `ADD_CPU` | ホスト | `{ count?: number, strategy?: CpuStrategy, fixed_hands?: Hand[] }`（開発/デモ用。ロビーのみ。既定 `count=1` / `strategy="RANDOM"`。`strategy="FIXED"` 時は `fixed_hands` 必須（1〜50 手）。`ALLOW_CPU=false` は `CPU_NOT_ALLOWED`。定員（20人）を超える追加は枠の残り分のみ追加せず `ROOM_FULL` を返す（接続は閉じない）） |
| `UPDATE_CPU` | ホスト | `{ player_id: string, fixed_hands: Hand[] }`（開発/デバッグ用。ロビーのみ。対象 CPU の手列を `FIXED` に切替・上書き。1〜50 手。`ALLOW_CPU=false` は `CPU_NOT_ALLOWED`） |
...
- `PlayerView`: `{ player_id, display_name, is_host: boolean, connection_state: "CONNECTED" | "DISCONNECTED", is_spectator: boolean, is_cpu: boolean, cpu_strategy?: CpuStrategy | null, cpu_fixed_hands?: Hand[] }`（`token` は**含めない**。CPU は常に `is_cpu=true` / `connection_state="CONNECTED"`、UI はロボットバッジで区別する。`cpu_strategy` / `cpu_fixed_hands` は CPU のみ。ホストはロビーで手列を編集する UI に用いる）
...
- **並行性**: ルーム状態への書き込みは**ルーム単位の単一ロック（`asyncio.Lock`）で全て直列化**する。判定（「全員提出」と「締切到達」の競合による二重判定防止）だけでなく、`JOIN` / `LEAVE` / `ADD_CPU` / `UPDATE_CPU` / `REMOVE_CPU` / `UPDATE_SETTINGS` / `START_GAME` などロビー操作も同じルーム状態を並行に書き換えうるため、これらも同一ロック配下で処理し、定員チェック・FSM 遷移・ホスト判定のレースを防ぐ（詳細は §7.1）。
...
| `CPU_NOT_ALLOWED` | `ALLOW_CPU=false` の環境で CPU 操作を実行 | `ADD_CPU`/`REMOVE_CPU`/`UPDATE_CPU` | — |
...
- **Player**: `player_id`, `token`(検証用。CPU は `null`), `display_name`(CPU は `CPU-1` 等の自動採番), `connection_state`(CONNECTED / DISCONNECTED。CPU は常に CONNECTED), `joined_at`, `is_cpu`(bool), `cpu_strategy`(`CpuStrategy` / 人間は `null`), `cpu_fixed_hands`(`Hand[]`・`FIXED` 時のみ。ロビーで設定), `cpu_fixed_hand_index`(int・次に出す手のインデックス。`START_GAME` で 0 にリセット)
...
- **永続化方針（MVP）**: 途中経過はインメモリのみ保持し、**Match 終了時の確定結果＋最終スコア**を `match_history` コレクションに保存する（**ラウンド単位の手・判定ルールは現状未保存**。再発調査用のラウンドログは `TODO.md` Phase 6 Step 5 / 本節 §5.2 を参照）。MVP では**ルーム単位のマッチ履歴のみ**を対象とし（`room_code` で参照）、`display_name` 横断のグローバルなスコアボード集計は後フェーズ（アカウント連携時）に設計する。
  - `match_history` ドキュメント例: `{ _id, room_code, match_id, rule_type, players: [{ player_id, display_name, is_cpu }], winner_ids, scores, started_at, ended_at }`（将来 `rounds: RoundLogEntry[]` を追加予定・§5.2）
  ...
  - **読み取り（Phase 5）**: `GET /rooms/{code}/matches`（§3.1）が MongoDB から一覧を返す。**インメモリのルーム状態とは独立**。ラウンド単位のリプレイ・グローバルスコアボード集計は Phase 6 Step 5 まで対象外。

### 5.2 ラウンドログ（計画・未実装）

バグ再発調査（例: 少数派→通常移行後の判定ずれ）のため、**サーバー権威のラウンド単位ログ**をマッチ終了時に永続化する方針（`TODO.md` Phase 6 Step 5）。

| 項目 | 方針 |
|---|---|
| **記録タイミング** | `RoundRunner` がラウンド判定を確定した直後（標準区画・TOURNAMENT 区画の双方）。`_judge_and_progress` の結果を正本とする |
| **保持（ライブ）** | `Match.round_log: RoundLogEntry[]` に append（`begin_round` で上書きされる `current_round` とは別） |
| **永続化** | 既存 `match_history` 書き込み時に `rounds` フィールドとして同梱（ラウンドごとの MongoDB insert は MVP では不要） |
| **WS プロトコル** | 変更しない（クライアント向けは既存 `ROUND_RESULT` のまま）。調査用の `judging_rule` 等はログ／REST のみ |
| **責務分割** | エントリ組み立ては `game/round_log.py`（純粋）、追記は `RoundRunner`、永続化は `core/match_history.py` |

**`RoundLogEntry` に含める想定フィールド**（実装時に Pydantic v2 で定義）:

- 識別: `round_no`, `segment_id`（TOURNAMENT 用・他は `null`）, `judged_at`（UTC ISO8601）
- 判定コンテキスト: `match_rule_type`, `judging_rule`（実際に使った判定: `MINORITY` / `NORMAL` / `BOSS` 等）, `switched_to_normal_finish`, `draw_round_count`, 関連 `MatchConfig` のスナップショット（少数派閾値・タイミング等）
- プレイヤー: 判定前 `alive_player_ids`, `hands: [{ player_id, display_name, hand, is_cpu }]`（未提出は手なしを明示）
- 結果: `is_draw`, `winner_ids`, `eliminated_ids`, `alive_after`, 最終ラウンド時は `match_ended` / `match_end_reason`

**任意（Phase B）**: 開発時のみ有効な構造化 `logging`（`MATCH_ROUND_LOG` 等）、`GET /rooms/{code}/matches/{match_id}` で `rounds` 返却、ロビー履歴パネルでのタイムライン表示。
...
| 状態 | 許可される操作 |
|---|---|
| WAITING(ロビー) | `JOIN` / `LEAVE` / `UPDATE_SETTINGS`(ホスト) / `ADD_CPU`・`UPDATE_CPU`・`REMOVE_CPU`(ホスト, `ALLOW_CPU` 時) / `START_GAME`(ホスト) |
...
- **CPU の自動提出**: CPU は alive な間、各 `ROUND_START` 時にサーバーが手を生成し（`cpu_strategy`）、自然さのため**短いランダム遅延（例 0.3〜1.5 秒）**で `SUBMIT_HAND` 相当の登録を行う。`RANDOM` は毎ラウンド乱数、`FIXED` は `cpu_fixed_hands` を順に再生し尽きたら先頭から循環（提出後に `cpu_fixed_hand_index` を進める）。`START_GAME` 時にインデックスを 0 にリセットする。これにより CPU は「全員提出」（§7）にカウントされ**ソロでも進行が止まらない**。CPU は未提出によるタイムアウト・切断・`LEAVE` を起こさない。CPU の追加/削除・手列の変更（`UPDATE_CPU`）は WAITING のみ可（IN_GAME 中は `INVALID_STATE`）。
...
- **直列化（二重判定防止・状態レース防止）**: 「全員提出」経路と「締切到達」経路は**ルーム単位の `asyncio.Lock`** 内で実行し、ロック取得後に「このラウンドが未判定か」を確認してから1回だけ判定する（後着の経路は no-op）。判定確定時にラウンド状態を `JUDGING` にし、以降の `SUBMIT_HAND` は `INVALID_STATE` で弾く。
  - **ロックのスコープは判定に限定しない**: §4「並行性」のとおり、`JOIN` / `LEAVE` / `ADD_CPU` / `UPDATE_CPU` / `REMOVE_CPU` / `UPDATE_SETTINGS` / `START_GAME` などルーム状態を書き換える操作はすべて同じルーム単位ロック配下で直列化する。これにより「`START_GAME` と `JOIN` の競合」「`ADD_CPU` 連打と定員チェックの競合」などのレースを防ぐ。ロック保持区間は状態更新＋（必要なら）ブロードキャスト準備までの短時間に限り、I/O（実際の WS 送信）は極力ロック外で行う。
...
- **開発/デモ用 CPU**: `.env` の `ALLOW_CPU`（既定 `true`）で有効化する。ホストはロビーで `ADD_CPU`/`UPDATE_CPU`/`REMOVE_CPU`（§4）により CPU を増減・手列を設定でき、一人でも全ゲームループ（提出→判定→結果→次ラウンド/終了）を確認できる。**本番は `ALLOW_CPU=false` を推奨**し、`.env.example` にも明記する。コード変更後は **uvicorn の再起動**（`--reload` 未使用時）が必要。
...
- **運用前提（MVP）**:
  ...
  - **トランスポート暗号化**: トークンを WS の `JOIN` で平文送信するため、**本番は `wss`/HTTPS を必須**とする（リバースプロキシ等で TLS 終端）。手順は `DEPLOY.md`（`TODO.md` Phase 7 Step 1）。
