...
- [`docs/TODO.md`](docs/TODO.md) — 実装フェーズ（Phase 1–9）
- [`docs/DEPLOY.md`](docs/DEPLOY.md) — 公開デモ・本番運用（Caddy + Cloudflare Tunnel）
...
## 公開デモ（自宅 PC + Cloudflare Tunnel）

研究室等への常時公開は **本番ビルド + Caddy + 名前付き Cloudflare Tunnel** を使う（`npm run dev` は使わない）。
手順・設定テンプレート・秘密情報の扱いは [`docs/DEPLOY.md`](docs/DEPLOY.md) を参照。

## 実装状況

- [x] **Phase 1**: 環境構築・シンプル REST API（ルーム作成/参加、トークン発行、レート制限、品質ツール・CI）
- [x] **Phase 2**: WebSocket・ゲームループ（MVP・NORMAL ルール）。Redis 実装（Step 3）は任意のため未着手
- [x] **Phase 3**: 特殊ルール（判定アルゴリズム + **ランタイム統合 R0–R6** 完了。ロビー・ゲーム画面から MINORITY / BOSS / TOURNAMENT をプレイ可能）
- [x] **Phase 4**: フロントエンド統合（WS・全画面・CPU UI・再接続/締切/ホスト移譲）
- [x] **Phase 5**: 対戦履歴の読み取り（`GET /rooms/{code}/matches`）とロビー履歴 UI（SWR / `MatchHistoryPanel`）

**MVP は完了**（Phase 1–5 + MVP 残タスク）。今後の実装ロードマップは [`docs/TODO.md`](docs/TODO.md) の **Phase 6 以降**を参照。

| Phase | 概要 | 状態 |
|-------|------|------|
| **6** | 品質・UX・E2E（reducer テスト、エラー表示、ラウンドログ、任意 Playwright） | Step 1–2 完了 |
| **7** | 本番デプロイ手順・設定チェックリスト（[`docs/DEPLOY.md`](docs/DEPLOY.md)） | Step 1–2 完了 |
| **8** | Redis / 水平スケール | 任意 |
| **9** | アカウント・グローバル機能・ゲーム拡張 | 将来 |
...
- 開発/デモ用 CPU は `ALLOW_CPU`（`.env`）で切替（本番は無効化推奨）。ロビーでランダム CPU または手列指定（`FIXED`）を追加でき、バグ再現・ソロ検証に使える（`ARCHITECTURE.md` §4/§6）。
