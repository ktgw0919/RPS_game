...
- [x] **Phase 3**: 特殊ルールの**判定アルゴリズム**（`game/rules/*`・`draw_resolution.py`・単体テスト）。**ランタイム統合**（`RoundRunner` / `ws.py` / フロント）は [`docs/TODO.md`](docs/TODO.md) Step R0–R6・[`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) §5.1 参照
- [x] **Phase 4**: フロントエンド統合（WS・全画面・CPU UI・再接続/締切/ホスト移譲）
- [x] **Phase 5**: 対戦履歴の読み取り（`GET /rooms/{code}/matches`）とロビー履歴 UI（SWR / `MatchHistoryPanel`）

**MVP 残タスク**（[`docs/TODO.md`](docs/TODO.md) 末尾）: QR 共有・ルーム操作 UI・（任意）フロント E2E。

**特殊ルールのランタイム統合**（[`docs/TODO.md`](docs/TODO.md) Step R0–R6）: モデル拡張 → `ws.py` → MINORITY → BOSS → TOURNAMENT → WS テスト → フロント UI。
